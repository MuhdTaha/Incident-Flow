...
| Invite email never arrives | Production needs `MAILJET_API_KEY`, `MAILJET_API_SECRET`, and a **verified** `MAILJET_SENDER_EMAIL` on the **API** service (not only the worker). Check spam. After this deploy, use **Copy invite link** on the success screen. Locally, open Mailhog at http://localhost:8025 — invites are not sent through Gmail. |
